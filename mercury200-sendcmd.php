#!/usr/bin/env php-cli
<?php
/**
* Send command to energy power meter 'Mercury 200' and print formatted output
* https://github.com/mrkrasser/MercuryStats
*
*
* @author Sergey Krasnov <me@sergeykrasnov.ru>
*/
ini_set('max_execution_time', 10);

// Calculate modbus crc 16
// from http://www.php.net/manual/ru/function.crc32.php#64127
function crc16_modbus($string) {
    $auchCRCHi=array( );
    $auchCRCLo=array(    );
    $length	= strlen($string);
    $uchCRCHi   = 0xFF;
    $uchCRCLo   = 0xFF;
    $uIndex     = 0;
    for ($i=0;$i<$length;$i++) {
        $uIndex   	= $uchCRCLo ^ ord(substr($string,$i,1));
        $uchCRCLo   = $uchCRCHi ^ $auchCRCHi[$uIndex];
        $uchCRCHi   = $auchCRCLo[$uIndex] ;
    }
    return(chr($uchCRCLo).chr($uchCRCHi));
}

// Format input string as nice hex
function nice_hex($str) {
    return strtoupper(implode(' ',str_split($str,2)));
}

// Show help if need
if ($argc != 3) {
	echo "\nSent commands to energy power meter 'Mercury 200' via serial interface\n";	
	echo "https://github.com/mrkrasser/MercuryStats\n";	
	echo "\nUsage: ".$argv[3]." <address> <command>\n";
	echo "\taddress: energy power device address (in hex)\n";
	echo "\tcommand: command to sent (in hex)\n";
	echo "Example: ".$argv[0]." 06475E 63\n\n";
	die();
}

// Parameters for port
exec('/bin/stty -F /dev/ttyUSB0 cs8 9600 ignbrk -brkint -icrnl -imaxbel -opost -onlcr -isig -icanon -iexten -echo -echoe -echok -echoctl -echoke noflsh -ixon -crtscts');

// Open port
$fp = fopen("/dev/ttyUSB0", "r+");
if (!$fp) {
	echo "Error opening port";
	die();
}

// Format command string and sent it to port
$cmd = '00'.$argv[1].$argv[2];
$cmd .= bin2hex(crc16_modbus(hex2bin($cmd)));
fwrite($fp, hex2bin($cmd));

// Read answer from port
$result = '';
$c = '';
stream_set_blocking($fp,0);
$timeout=microtime(1)+0.5;
while (microtime(1)<$timeout) {
	$c=fgetc($fp);
	if($c === false){
			usleep(5);
			continue;
	}
	$result .= $c;
}
fclose($fp);

// Split answer data on parts
$crc_answer = substr($result,-2);                // crc16 in answer
$addr = substr($result,1,3);   			 // address of power device
$answer_cmd = substr($result,4,1);              // answered command
$answer = substr($result,5,-2);                  // answer string

// Check CRC
$crc_check = 'FAIL';
if (crc16_modbus(substr($result,0,-2))==$crc_answer) {
	$crc_check = 'OK';
}

// Formatted output
echo 'ADDRESS: '.nice_hex(bin2hex($addr))."\n";
echo 'COMMAND: '.nice_hex(bin2hex($answer_cmd))."\n";
echo 'ANSWER:  '.nice_hex(bin2hex($answer))."\n";
echo 'CRC:     '.$crc_check."\n"; 
echo '---------'."\n";
echo 'Full request: '.nice_hex($cmd)."\n";
echo 'Full answer:  '.nice_hex(bin2hex($result))."\n\n";
